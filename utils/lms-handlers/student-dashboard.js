...
function isActiveEnrollment(status) {
  return ACTIVE_ENROLLMENT_STATUSES.has(norm(status));
}

function isApprovedOrder(status) {
  return ["da duyet", "approved", "active"].includes(norm(status));
}

function isRejectedOrder(status) {
  return ["tu choi", "da huy", "huy", "rejected", "cancelled", "canceled"].includes(norm(status));
}
...
async function resolveEmail(req) {
  ...
  if (credential) {
    const email = String(await verifyGoogleIdToken(credential) || "").trim().toLowerCase();
  ...
  if (lmsHeaders.lmsSessionId && lmsHeaders.lmsDeviceId) {
    ...
    if (access?.ok && access.email) return String(access.email).trim().toLowerCase();
  }
  if (token) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [{ data: orders, error: orderError }, { data: enrollments, error: enrollmentError }] = await Promise.all([
      supabase
        .from("orders")
        .select("id,course_slug,course_title,status,delivery_mode,created_at,updated_at")
    ...
    const latestOrderBySlug = new Map();
    for (const order of orders || []) {
      const slug = String(order.course_slug || "").trim();
      if (slug && !latestOrderBySlug.has(slug)) latestOrderBySlug.set(slug, order);
    }
    ...
    for (const enrollment of enrollments || []) {
      ...
      if (!slug) continue;
      const existing = enrollmentBySlug.get(slug);
      if (!existing || new Date(enrollment.updated_at || enrollment.created_at || 0) > new Date(existing.updated_at || existing.created_at || 0)) {
        enrollmentBySlug.set(slug, enrollment);
      }
    }

    const slugs = [...new Set([...latestOrderBySlug.keys(), ...enrollmentBySlug.keys()])];
    let courseRows = [];
    if (slugs.length) {
      const { data, error } = await supabase
        .from("courses")
        .select("slug,title,image_url,active,is_published,delivery_mode,expected_start_date,raw_data")
        .in("slug", slugs);
    ...
    const courses = slugs.map(slug => {
      const order = latestOrderBySlug.get(slug) || null;
      const enrollment = enrollmentBySlug.get(slug) || null;
      const course = courseBySlug.get(slug) || {};
      const raw = course.raw_data || {};
      const enrollmentActive = Boolean(enrollment && isActiveEnrollment(enrollment.status));
      const orderApproved = Boolean(order && isApprovedOrder(order.status));
      const orderRejected = Boolean(order && isRejectedOrder(order.status));
      const ready = enrollmentActive && course.active !== false && course.is_published === true;

      let state = "pending_approval";
      if (ready) state = "ready";
      else if (enrollmentActive || orderApproved) state = "approved_waiting_content";
      else if (orderRejected) state = "rejected";
      else if (!order && enrollment) state = enrollmentActive ? "approved_waiting_content" : "inactive";

      const title = String(raw.studentDisplayTitle || course.title || order?.course_title || slug).trim() || slug;
      return {
        slug,
        title,
        imageUrl: String(course.image_url || ""),
        deliveryMode: String(course.delivery_mode || order?.delivery_mode || "lms"),
        isPublished: course.is_published === true,
        active: course.active !== false,
        expectedStartDate: course.expected_start_date || null,
        orderStatus: order?.status || null,
        enrollmentStatus: enrollment?.status || null,
        state,
        updatedAt: order?.updated_at || enrollment?.updated_at || order?.created_at || enrollment?.created_at || null
      };
