...
begin
  ...
  -- Narrow, one-way exception for a released READY R2 video that did not have
  -- a thumbnail at publish time. Only thumbnail_asset_id may change, and only
  -- NULL -> READY private-R2 JPEG image. Every other content field stays
  -- immutable; replacement/removal of an existing thumbnail remains blocked.
  if old.type = 'video'
     and old.provider = 'r2'
