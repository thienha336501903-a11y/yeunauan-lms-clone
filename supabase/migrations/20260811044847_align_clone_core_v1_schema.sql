...
-- COURSES: restore V1 defaults/nullability and slug uniqueness.
...
-- LESSONS: restore V1 defaults/nullability.
...
-- ORDERS: restore shared V1 constraints/defaults; preserve Clone-only columns.
...
-- LESSON PROGRESS: restore V1 defaults/nullability.
...
-- Add missing V1 constraints idempotently.
do $$
begin
  if not exists (
  ...
  if not exists (
    select 1 from pg_constraint
    where conname='lessons_course_id_fkey' and conrelid='public.lessons'::regclass
  ) then
  ...
  if not exists (
    select 1 from pg_constraint
    where conname='lessons_course_slug_lesson_no_key' and conrelid='public.lessons'::regclass
  ) then
    alter table public.lessons add constraint lessons_course_slug_lesson_no_key
      unique (course_slug, lesson_no);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname='lessons_kind_check' and conrelid='public.lessons'::regclass
  ) then
    alter table public.lessons add constraint lessons_kind_check
      check (kind = any (array['section'::text, 'lesson'::text]));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname='lessons_parent_section_id_fkey' and conrelid='public.lessons'::regclass
  ) then
    alter table public.lessons add constraint lessons_parent_section_id_fkey
      foreign key (parent_section_id) references public.lessons(id) on delete set null;
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname='orders_course_id_fkey' and conrelid='public.orders'::regclass
  ) then
  ...
  if not exists (
    select 1 from pg_constraint
    where conname='lesson_progress_email_lesson_id_key' and conrelid='public.lesson_progress'::regclass
  ) then
    alter table public.lesson_progress add constraint lesson_progress_email_lesson_id_key
      unique (email, lesson_id);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conname='lesson_progress_lesson_id_fkey' and conrelid='public.lesson_progress'::regclass
  ) then
