...
-- ── Service-role-only RPCs matching the current V1 system ───────────────────
...
begin
  ...
  select * into v_existing from student_active_sessions
...
begin
  ...
  on conflict (lower(trim(email))) do update
    set session_generation=student_session_controls.session_generation+1,
        ...
        reason=p_reason,updated_at=v_now;
  select coalesce(array_agg(student_session_id),array[]::text[]) into v_session_ids
  from student_active_sessions where lower(trim(email))=v_email and status='active';
  update student_active_sessions set status='admin_reset',logout_at=v_now,updated_at=v_now where lower(trim(email))=v_email and status='active';
  ...
  insert into admin_audit_logs(admin_email,action,target_email,metadata,created_at)
  values(nullif(lower(trim(coalesce(p_admin_email,''))),''),'reset_student_session_guard',v_email,jsonb_build_object('reason',p_reason,'studentSessions',v_student_count,'entryTokens',v_token_count,'lmsSessions',v_lms_count,'revokedBefore',v_now),v_now);
