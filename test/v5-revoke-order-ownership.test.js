...
test('a new V5 order cannot steal a still-usable entitlement owned by another grant', () => {
  assert.match(source, /orderEnrollment && isEnrollmentUsable\(orderEnrollment\)/);
  assert.match(source, /targetEnrollment && targetEnrollment\.id !== orderEnrollment\?\.id/);
  assert.match(source, /isEnrollmentUsable\(targetEnrollment\)/);
  assert.match(source, /v5_order_entitlement_identity_locked/);
  assert.match(source, /v5_enrollment_owned_by_other_grant/);
});

test('repurchase after revoke or expiry reactivates the enrollment with guarded ownership and clears old expiry', () => {
  assert.match(source, /status:\s*"active"/);
  assert.match(source, /expired_at:\s*null/);
  assert.match(source, /async function activateEnrollmentRow\(existing, payload\)/);
  assert.match(source, /guardNullable\(query, "source_order_id", existing\.source_order_id\)/);
  assert.match(source, /enrollment = await activateEnrollmentRow\(targetEnrollment, payload\)/);
  assert.doesNotMatch(source, /onConflict:\s*"email,course_slug"/);
});
